...
export function createSessionWithId(id, participants = [], question = null) {
  if (sessions.has(id)) {
    const s = sessions.get(id);
    // If question is provided and not set yet, attach it (idempotent upsert)
    if (question && !s.question) {
      s.question = sanitizeQuestion(question);
    }
    return { error: 'exists', session: s };
  }
...
function sanitizeQuestion(question) {
  ...
  const { id, _id, title, description, difficulty, topics, examples } = question;
  return {
    id: id || _id || null,
    title: title || null,
